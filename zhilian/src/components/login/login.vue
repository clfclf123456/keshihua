  ...
  <div class="login">
    ...
    <div class="main" v-else>
      ...
      <div class="userPassword">
        <div>密码：</div>
        <div class="input">
          <el-input v-model="registerInfo.password" type="password" placeholder="请输入密码"></el-input>
        </div>
      </div>
      <div class="userPassword">
        <div>学校：</div>
...
export default {
  ...
  methods: {
    ...
    submit () {
      ...
      window.sessionStorage.setItem('id', res.data.data)
    ...
    register () {
      this.$http.post('api/user/register', this.registerInfo).then(res => {
        if (res.data.code === 200) {
          this.$message({
            message: '注册成功',
            type: 'success'
...
<style scoped src="./login.less" lang="less"></style>
